...
fake_users_db = {
    "admin": {
        "token": "some_token_for_admin"
    },
    "user1": {
        "token": "some_other_token_for_user1"
    }
}
...
@app.get("/employees")
def get_all_employees(credentials: HTTPAuthorizationCredentials = Depends(security)):
    print("Received token:", credentials.credentials)
    for user in fake_users_db.values():
        if user["token"] == credentials.credentials:
            break
    else:
