...
def _call_gemini(prompt, api_key, max_tokens=1024):
    """
    Call Google Gemini via REST. Handles both key types automatically:

    AIza... keys  → Google AI Studio endpoint (?key= query param)
                    Get one at: https://aistudio.google.com/app/apikey

    AQ...   keys  → Vertex AI endpoint (Authorization: Bearer header)
                    These are OAuth2 access tokens issued when your Google
                    account is restricted from generating AIza keys.
                    Also set GEMINI_PROJECT_ID in settings.py.
                    Generate a fresh token with:
                      gcloud auth print-access-token
                    Note: AQ tokens expire after ~1 hour.
    """
    models_to_try = [
        "gemini-2.5-flash",
    ...
    is_aq_token = api_key.startswith("AQ")
    project_id  = getattr(settings, "GEMINI_PROJECT_ID", "").strip()
    last_error  = None

    for model in models_to_try:
        if is_aq_token:
            # ── Vertex AI endpoint (Bearer token auth) ──────────────────────
            if not project_id:
                raise Exception(
                    "GEMINI_PROJECT_ID is not set in settings.py. "
                    "AQ tokens require a GCP project ID. "
                    "Find yours at https://console.cloud.google.com and add it to settings.py."
                )
            ...
            # ── Google AI Studio endpoint (API key query param) ─────────────
            url = (
        ...
        try:
            ...
            # Vertex AI and AI Studio have slightly different response shapes
            ...
            if code in (401, 403):
                if is_aq_token:
                    raise Exception(
                        f"Gemini Bearer token rejected ({code}). "
                        "AQ tokens expire after ~1 hour. Generate a fresh one with: "
                        "  gcloud auth print-access-token\n"
                        "Then update GEMINI_API_KEY in settings.py."
                    )
                else:
                    raise Exception(
                        f"Gemini API key rejected ({code}). "
                        "Make sure GEMINI_API_KEY in settings.py is a valid "
                        "AIza... key from https://aistudio.google.com/app/apikey"
                    )
            # 404 = model not available on this endpoint, try next
            if code == 404:
                last_error = msg
                continue
            raise Exception(f"Gemini error {code}: {msg}")

    raise Exception(f"No Gemini model responded. Last error: {last_error}")
